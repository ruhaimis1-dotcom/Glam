      ...
      <header className="sticky top-0 z-20 border-b border-[#eadfda] bg-[#fbf8f5]/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8">
          ...
          <nav className="hidden items-center gap-7 text-sm text-[#745e70] md:flex">
            <a href="#discover" className="hover:text-[#8c285d]">
              اكتشفي
            </a>
            <a href="#salons" className="hover:text-[#8c285d]">
              الصالونات
            </a>
            <a href="#how" className="hover:text-[#8c285d]">
              كيف تعمل Glam؟
            </a>
          </nav>
          <Link
            to="/login"
            className="rounded-full border border-[#8c285d]/30 px-4 py-2 text-sm font-medium text-[#8c285d] hover:bg-[#f4e8ee]"
          >
            تسجيل الدخول
          </Link>
        </div>
        ...
        <section className="relative overflow-hidden px-5 pb-10 pt-8 sm:px-8 sm:pt-12">
          <div className="mx-auto grid max-w-7xl items-end gap-8 lg:grid-cols-[1.05fr_.95fr]">
            <div className="order-2 pb-4 lg:order-1 lg:pb-12">
              <div className="mb-5 inline-flex items-center gap-2 rounded-full bg-[#f4e8ee] px-3 py-1.5 text-sm text-[#8c285d]">
                <Sparkles className="size-4" /> جمالك، على وقتك
              </div>
              <h1 className="max-w-2xl text-4xl font-semibold leading-[1.2] tracking-tight sm:text-6xl">
                اكتشفي المكان الأقرب لذوقك.
              </h1>
              <p className="mt-5 max-w-xl text-lg leading-8 text-[#745e70]">
                قارني الأعمال الحقيقية، التقييمات، الأسعار والمواعيد المتاحة في مكان واحد.
              </p>
              <div className="mt-8 flex max-w-xl items-center gap-2 rounded-2xl border border-[#eadfda] bg-white p-2 shadow-sm">
                ...
                <button
                  type="button"
                  onClick={discover}
                  className="rounded-xl bg-[#5A1835] px-5 py-3 font-semibold text-white hover:bg-[#3D0F26]"
                >
                  اكتشفي
                </button>
              </div>
              <div className="mt-6 flex flex-wrap gap-3 text-sm text-[#745e70]">
                <span className="flex items-center gap-1">
                  <MapPin className="size-4 text-[#8c285d]" /> الرياض
                </span>
                <span>•</span>
            ...
            <div className="order-1 relative h-[360px] overflow-hidden rounded-[2rem] lg:order-2 lg:h-[500px]">
              <img
                ...
                alt="تجربة عناية داخل صالون عصري"
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
            {CATEGORIES.map((category) => (
              <button
                ...
                className={`rounded-2xl border border-[#eadfda] bg-white p-4 text-center transition hover:-translate-y-0.5 hover:border-[#5A1835]/40 hover:shadow-sm ${selectedCategory === category.id ? "ring-2 ring-[#5A1835]" : ""}`}
              >
                {(() => {
                  const Icon = categoryIcons[category.id];
                  return <Icon className="mx-auto size-6 text-[#8c285d]" strokeWidth={1.7} />;
                })()}
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              {INTENTS.map((intent) => (
                <button
                  ...
                  className={`rounded-2xl border border-[#eadfda] bg-[#fbf8f5] p-5 text-right hover:border-[#5A1835]/40 ${selectedIntent === intent.id ? "ring-2 ring-[#5A1835]" : ""}`}
                >
                  {(() => {
                    const Icon = intentIcons[intent.id];
                    return <Icon className="size-6 text-[#8c285d]" strokeWidth={1.7} />;
                  })()}
