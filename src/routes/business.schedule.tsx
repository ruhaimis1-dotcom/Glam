    ...
    getCurrentOrganization().then(async ({ organizationId }) => {
      ...
      setRows(
        data.map((a) => ({
          ...
          label: `${new Date(a.starts_at).toLocaleTimeString("ar-SA", { hour: "numeric", minute: "2-digit" })}${a.ends_at ? `–${new Date(a.ends_at).toLocaleTimeString("ar-SA", { hour: "numeric", minute: "2-digit" })}` : ""} — ${a.service_name}${a.specialist_name ? ` — ${a.specialist_name}` : ""}${a.buffer_minutes ? ` · تجهيز ${a.buffer_minutes} د` : ""}`,
